...
export async function updateRaceData(): Promise<{
  season: number;
  matchedCount: number;
  ...
  try {
    console.log('🔄 Updating race data from F1 API...');
    ...
    console.log(
      `✅ Races updated for ${year}. matched: ${result.matchedCount}, modified: ${result.modifiedCount}, upserted: ${result.upsertedCount}`
    );
    ...
    console.error('❌ Failed to update race data:', error.message);
    throw error;
  }
}

export async function dropLegacyIndexes(): Promise<void> {
  try {
